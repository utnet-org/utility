...
fn sync_nodes() {
    heavy_test(|| {
        ...
        let (genesis, genesis_block, unc1, unc2) = setup_configs();
...
fn sync_state_pledge_change() {
    heavy_test(|| {
        ...
        unc2.client_config.min_block_production_delay = Duration::from_millis(200);
        ...
        run_actix(async {
            let framework::UncNode { client: client1, view_client: view_client1, .. } =
            ...
            let unpledge_transaction = SignedTransaction::pledge(
                ...
                unc1.validator_signer.as_ref().unwrap().public_key(),
                genesis_hash,
            );
            actix::spawn(
                client1
                ...
                Box::new(move |_ctx| {
                    ...
                    let actor = actor.then(move |res| {
                        ...
                        if !started_copy.load(Ordering::SeqCst) && latest_height > 2 * epoch_length
                        {
                            ...
                            let framework::UncNode { view_client: view_client2, arbiters, .. } =
