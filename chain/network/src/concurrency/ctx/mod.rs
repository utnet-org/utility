...
struct Inner {
    ...
    deadline: unc_async::time::Deadline,
...
impl Ctx {
    ...
    pub(crate) fn new(clock: unc_async::time::Clock) -> Ctx {
        return Ctx(Arc::new(Inner {
            canceled: signal::Once::new(),
            deadline: unc_async::time::Deadline::Infinite,
            clock,
            _parent: None,
        }));
    }
    ...
    pub fn sub(&self, deadline: unc_async::time::Deadline) -> Ctx {
...
pub fn get_deadline() -> unc_async::time::Deadline {
    local().0.deadline
}
...
pub fn run_canceled<F: Future>(f: F) -> impl Future<Output = F::Output> {
    let ctx = local().sub(unc_async::time::Deadline::Infinite);
    ctx.cancel();
    CtxFuture { ctx, inner: f }
}
