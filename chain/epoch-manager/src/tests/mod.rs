...
fn test_power_validator() {
    ...
    let expected0 = epoch_info_with_num_seats(
        ...
        vec![],
        vec![],
        power_validators.into_iter().collect(),
        BTreeMap::new(),
        vec![],
    ...
    let compare_epoch_infos = |a: &EpochInfo, b: &EpochInfo| -> bool {
        a.validators_iter().eq(b.validators_iter())
            ...
            && a.pledge_change() == b.pledge_change()
    ...
    record_block(
        ...
        h[1],
        1,
        vec![power("test2".parse().unwrap(), amount_powered)],
    ...
    let expected3 = epoch_info_with_num_seats(
        ...
        vec![],
        vec![],
        vec![("test1".parse().unwrap(), amount_powered)].into_iter().collect(),
        vec![("test1".parse().unwrap(), amount_pledged)].into_iter().collect(),
        vec![],
...
#[test]
fn test_validator_change_of_pledge() {
    let amount_pledged = 1_000_000;
    let amount_powered: Power = 1_000_000;
    let fishermen_threshold = 100;
    let pledge_validators = vec![
        ("test1".parse().unwrap(), amount_pledged),
    ...
    check_validators(&epoch_info, &[("test2", amount_pledged)]);
    ...
    matches!(
        ...
        Some(ValidatorKickoutReason::NotEnoughPower { power: 10, .. })
    );
}
...
fn test_fork_finalization() {
    ...
    let pledge_validators = vec![
        ("test1".parse().unwrap(), amount_pledged),
        ("test2".parse().unwrap(), amount_pledged),
        ("test3".parse().unwrap(), amount_pledged),
    ];
    let power_validators = vec![
        ("test1".parse().unwrap(), amount_powered),
        ("test2".parse().unwrap(), amount_powered),
     ...
     -> Vec<CryptoHash> {
        ...
        for (i, curr_block) in hashes.iter().enumerate().skip(2) {
            ...
            if validator_accounts.iter().any(|v| *v == account_id) {
                record_block(epoch_manager, prev_block, *curr_block, height, vec![], vec![]);
    ...
    record_block(
        ...
        h[1],
        1,
        vec![power("test4".parse().unwrap(), amount_powered)],
        vec![pledge("test4".parse().unwrap(), amount_pledged)],
    );
...
fn test_one_validator_kickout() {
    let amount_pledged = 1_000;
    let amount_powered: Power = 1_000;
    let mut epoch_manager = setup_default_epoch_manager(
        vec![("test1".parse().unwrap(), amount_powered)],
        vec![("test1".parse().unwrap(), amount_pledged)],
        2,
        1,
        1,
        0,
    ...
    record_block(&mut epoch_manager, CryptoHash::default(), h[0], 0, vec![], vec![]);
    record_block(&mut epoch_manager, h[0], h[2], 2, vec![], vec![]);
    record_block(&mut epoch_manager, h[2], h[4], 4, vec![], vec![]);
    record_block(&mut epoch_manager, h[4], h[5], 5, vec![], vec![]);
    let epoch_id = epoch_manager.get_next_epoch_id(&h[5]).unwrap();
    let epoch_info = epoch_manager.get_epoch_info(&epoch_id).unwrap();
    check_validators(&epoch_info, &[("test1", amount_pledged)]);
    check_fishermen(&epoch_info, &[]);
    check_kickout(&epoch_info, &[]);
    check_pledge_change(&epoch_info, vec![("test1".parse().unwrap(), amount_pledged)]);
}
...
fn test_validator_kickout() {
    ...
    for (i, curr_block) in h.iter().enumerate().skip(1) {
        ...
        if block_producer.account_id() == "test2" && epoch_id == init_epoch_id {
            ...
            record_block(&mut epoch_manager, prev_block, *curr_block, height, vec![], vec![]);
            prev_block = *curr_block;
        }
    }
...
#[test]
fn test_validator_unpledge() {
    let store = create_test_store();
    let config = epoch_config(2, 1, 2, 0, 90, 60, 0);
    let amount_pledged = 1_000_000;
    let pledge_validators = vec![
        pledge("test1".parse().unwrap(), amount_pledged),
        pledge("test2".parse().unwrap(), amount_pledged),
    ];
    let amount_powered: Power = 1_000_000;
    let power_validators = vec![
    ...
    check_kickout(&epoch_info, &[("test1", ValidatorKickoutReason::Unpowered)]);
    check_reward(
        &epoch_info,
        vec![
            ("test1".parse().unwrap(), 0),
    ...
    record_block(&mut epoch_manager, h[3], h[4], 4, vec![], vec![]);
    record_block(&mut epoch_manager, h[4], h[5], 5, vec![], vec![]);
    let epoch_id = epoch_manager.get_next_epoch_id(&h[5]).unwrap();
    let epoch_info = epoch_manager.get_epoch_info(&epoch_id).unwrap();
    check_validators(&epoch_info, &[("test2", amount_pledged)]);
    check_fishermen(&epoch_info, &[]);
    check_pledge_change(&epoch_info, vec![("test2".parse().unwrap(), amount_pledged)]);
    check_kickout(&epoch_info, &[]);
    check_reward(
        &epoch_info,
        vec![
    ...
    record_block(&mut epoch_manager, h[5], h[6], 6, vec![], vec![]);
    record_block(&mut epoch_manager, h[6], h[7], 7, vec![], vec![]);
...
fn test_double_sign_slashing1() {
    ...
    let h = hash_range(10);
    record_block(&mut epoch_manager, CryptoHash::default(), h[0], 0, vec![], vec![]);
    record_block(&mut epoch_manager, h[0], h[1], 1, vec![], vec![]);
    record_block_with_slashes(
        &mut epoch_manager,
        h[1],
        h[2],
    ...
    assert_eq!(
        epoch_info
            .validators_iter()
            .map(|v| (v.account_id().clone(), v.power()))
            .collect::<Vec<_>>(),
        vec![("test2".parse().unwrap(), amount_powered)],
    );
    assert_eq!(
...
/// If all current validator try to unpledge, we disallow that.
#[test]
fn test_all_validators_unpledge() {
    ...
    let powered: Power = 1_000;
    let power_validators = vec![
        ("test1".parse().unwrap(), powered),
        ("test2".parse().unwrap(), powered),
    ...
    record_block(
        ...
        1,
        vec![
            power("test1".parse().unwrap(), 0),
            power("test2".parse().unwrap(), 0),
            power("test3".parse().unwrap(), 0),
        ],
        vec![
            pledge("test1".parse().unwrap(), 0),
            pledge("test2".parse().unwrap(), 0),
            pledge("test3".parse().unwrap(), 0),
        ],
    );
    record_block(&mut epoch_manager, h[1], h[2], 2, vec![], vec![]);
    let _next_epoch = epoch_manager.get_next_epoch_id(&h[2]).unwrap();
}
...
fn test_validator_reward_one_validator() {
    ...
    let mut validator_pledges = HashMap::new();
    validator_pledges.insert("test2".parse().unwrap(), pledge_amount);
    ...
    check_validators(&epoch_info, &[("test2", pledge_amount + test2_reward)]);
    check_fishermen(&epoch_info, &[("test1", test1_pledge_amount)]);
    check_pledge_change(
        &epoch_info,
        vec![
            ("test1".parse().unwrap(), test1_pledge_amount),
            ("test2".parse().unwrap(), pledge_amount + test2_reward),
        ],
    );
    check_kickout(&epoch_info, &[]);
    check_reward(
...
#[test]
fn test_validator_reward_weight_by_pledge() {
    let pledge_amount1 = 1_000_000;
    let pledge_amount2 = 500_000;
    let validators = vec![
        ("test1".parse().unwrap(), pledge_amount1),
        ("test2".parse().unwrap(), pledge_amount2),
    ];
    let epoch_length = 2;
    let total_supply = (pledge_amount1 + pledge_amount2) * validators.len() as u128;
    let reward_calculator = RewardCalculator {
        max_inflation_rate: Ratio::new(5, 100),
        num_blocks_per_year: 50,
        epoch_length,
    ...
    let mut validators_pledges = HashMap::new();
    validators_pledges.insert("test1".parse().unwrap(), pledge_amount1);
    validators_pledges.insert("test2".parse().unwrap(), pledge_amount2);
    let (validator_reward, inflation) = reward_calculator.calculate_reward(
        validator_online_ratio,
        &validators_pledges,
        total_supply,
    ...
    check_validators(
        &epoch_info,
        &[("test1", pledge_amount1 + test1_reward), ("test2", pledge_amount2 + test2_reward)],
    );
    check_fishermen(&epoch_info, &[]);
    check_pledge_change(
        &epoch_info,
        vec![
            ("test1".parse().unwrap(), pledge_amount1 + test1_reward),
            ("test2".parse().unwrap(), pledge_amount2 + test2_reward),
        ],
    );
    check_kickout(&epoch_info, &[]);
    check_reward(
...
fn test_reward_multiple_shards() {
    let pledge_amount = 1_000_000;
    let validators =
        vec![("test1".parse().unwrap(), pledge_amount), ("test2".parse().unwrap(), pledge_amount)];
    let epoch_length = 10;
    let total_supply = pledge_amount * validators.len() as u128;
    let reward_calculator = RewardCalculator {
        max_inflation_rate: Ratio::new(5, 100),
    ...
    let mut validators_pledges = HashMap::new();
    validators_pledges.insert("test1".parse().unwrap(), pledge_amount);
    validators_pledges.insert("test2".parse().unwrap(), pledge_amount);
    let (validator_reward, inflation) = reward_calculator.calculate_reward(
        validator_online_ratio,
        &validators_pledges,
        total_supply,
...
#[test]
fn test_unpledge_and_then_change_pledge() {
    let amount_powered = 1_000_000;
    let validators = vec![
        ("test1".parse().unwrap(), amount_powered),
        ("test2".parse().unwrap(), amount_powered),
    ];
    ...
    // test1 unpledges in epoch 1, and should be kicked out in epoch 3 (validators stored at h2).
    record_block(&mut epoch_manager, h[0], h[1], 1, vec![power("test1".parse().unwrap(), 0)]);
    record_block(
        &mut epoch_manager,
        h[1],
        h[2],
        2,
        vec![power("test1".parse().unwrap(), amount_powered)],
    );
    record_block(&mut epoch_manager, h[2], h[3], 3, vec![]);
    let epoch_id = epoch_manager.get_next_epoch_id(&h[3]).unwrap();
    assert_eq!(epoch_id, EpochId(h[2]));
    let epoch_info = epoch_manager.get_epoch_info(&epoch_id).unwrap();
    check_validators(&epoch_info, &[("test1", amount_powered), ("test2", amount_powered)]);
    check_fishermen(&epoch_info, &[]);
    check_pledge_change(
        &epoch_info,
        vec![
...
fn test_epoch_info_aggregator_data_loss() {
    let power_amount = 1_000_000;
    let validators =
        vec![("test1".parse().unwrap(), power_amount), ("test2".parse().unwrap(), power_amount)];
    let epoch_length = 5;
    let mut em = setup_epoch_manager(
        validators,
        epoch_length,
    ...
    assert_eq!(
        ...
        vec![
            power("test1".parse().unwrap(), power_amount - 1),
            power("test2".parse().unwrap(), power_amount + 10)
        ]
        .into_iter()
...
fn test_epoch_info_aggregator_reorg_beginning_of_epoch() {
    ...
    record_block_with_final_block_hash(
        ...
        h[4],
        6,
        vec![power("test2".parse().unwrap(), power_amount - 100)],
    );
    // reorg
...
fn test_compare_epoch_id() {
    ...
    record_block(
        ...
        h[2],
        2,
        vec![power("test1".parse().unwrap(), amount_pledged)],
    );
    record_block(&mut epoch_manager, h[2], h[3], 3, vec![]);
...
fn test_fishermen() {
    ...
    check_validators(&epoch_info, &[("test1", pledge_amount), ("test2", pledge_amount)]);
...
fn test_fishermen_unpledge() {
    ...
    // fishermen unpledge
    record_block(&mut em, h[0], h[1], 1, vec![power("test2".parse().unwrap(), 0)]);
    record_block(&mut em, h[1], h[2], 2, vec![power("test3".parse().unwrap(), 1)]);

    let epoch_info = em.get_epoch_info(&EpochId(h[2])).unwrap();
    check_validators(&epoch_info, &[("test1", pledge_amount)]);
    check_fishermen(&epoch_info, &[]);
    check_pledge_change(
        &epoch_info,
        vec![
    ...
    assert_eq!(
        ...
        &ValidatorKickoutReason::Unpowered
    );
    matches!(
        kickout.get(AccountIdRef::new_or_panic("test3")),
        Some(ValidatorKickoutReason::NotEnoughPower { .. })
    );
}
...
fn test_finalize_epoch_large_epoch_length() {
    ...
    assert_eq!(
        epoch_info.validators_iter().map(|v| v.account_and_power()).collect::<Vec<_>>(),
        vec![("test1".parse().unwrap(), power_amount), ("test2".parse().unwrap(), power_amount)],
    );
    assert_eq!(
        epoch_info.pledge_change(),
...
fn test_kickout_set() {
    ...
    record_block(
        ...
        h[1],
        1,
        vec![power("test2".parse().unwrap(), power_amount)],
    );
    record_block(&mut epoch_manager, h[1], h[2], 2, vec![power("test2".parse().unwrap(), 0)]);
    ...
    assert_eq!(
        epoch_info1.pledge_change().clone(),
        change_power(vec![
            ("test1".parse().unwrap(), power_amount),
            ("test2".parse().unwrap(), 0),
            ("test3".parse().unwrap(), 10)
    ...
    check_validators(&epoch_info, &[("test1", power_amount), ("test2", power_amount)]);
...
fn test_epoch_height_increase() {
    ...
    let h = hash_range(5);
    record_block(&mut epoch_manager, CryptoHash::default(), h[0], 0, vec![]);
    record_block(&mut epoch_manager, h[0], h[2], 2, vec![power("test1".parse().unwrap(), 223)]);
...
fn test_unpledge_slash() {
    ...
    record_block(
        ...
        h[4],
        4,
        vec![power("test1".parse().unwrap(), power_amount)],
    );
    ...
    assert_eq!(
        ...
        Some(&ValidatorKickoutReason::Unpowered)
    );
    assert_eq!(
...
#[test]
/// Slashed with no unpledge in previous epoch: slashed for 3 epochs
fn test_no_unpledge_slash() {
    let power_amount = 1_000;
    let validators = vec![
        ("test1".parse().unwrap(), power_amount),
        ("test2".parse().unwrap(), power_amount),
...
fn test_slash_non_validator() {
    ...
    let epoch_info1 = epoch_manager.get_epoch_info(&EpochId(h[1])).unwrap(); // Unpledged
    let epoch_info2 = epoch_manager.get_epoch_info(&EpochId(h[2])).unwrap(); // -
...
#[test]
/// Slashed and attempt to repledge: proposal gets ignored
fn test_slash_repledge() {
    let pledge_amount = 1_000;
    let pledge_validators = vec![
        ("test1".parse().unwrap(), pledge_amount),
        ("test2".parse().unwrap(), pledge_amount),
    ...
    record_block(
        ...
        2,
        vec![power("test1".parse().unwrap(), power_amount)],
        vec![pledge("test1".parse().unwrap(), pledge_amount)],
    );
    record_block(&mut epoch_manager, h[2], h[3], 3, vec![], vec![]);
    record_block(
        &mut epoch_manager,
    ...
    let epoch_info2 = epoch_manager.get_epoch_info(&EpochId(h[2])).unwrap();
    assert!(epoch_info2.pledge_change().get(AccountIdRef::new_or_panic("test1")).is_none());
    let epoch_info4 = epoch_manager.get_epoch_info(&EpochId(h[4])).unwrap();
    assert!(epoch_info4.pledge_change().get(AccountIdRef::new_or_panic("test1")).is_some());
}
...
fn check_pledge_change(epoch_info: &EpochInfo, changes: Vec<(AccountId, u128)>) {
    assert_eq!(epoch_info.pledge_change(), &(changes.into_iter().collect()));
}
...
fn test_fisherman_kickout() {
    ...
    // - Finalize epoch T+1 => T+3 kicks test1 as fisherman without a record in pledge_change
    record_block(&mut epoch_manager, h[1], h[3], 3, vec![], vec![]);

    let epoch_info2 = epoch_manager.get_epoch_info(&EpochId(h[1])).unwrap();
    check_validators(&epoch_info2, &[("test2", pledge_amount), ("test3", pledge_amount)]);
    ...
    let epoch_info3 = epoch_manager.get_epoch_info(&EpochId(h[3])).unwrap();
    check_validators(&epoch_info3, &[("test2", pledge_amount), ("test3", pledge_amount)]);
...
fn test_protocol_version_switch() {
    ...
    for i in 2..6 {
        record_block(&mut epoch_manager, h[i - 1], h[i], i as u64, vec![], vec![]);
    }
...
fn test_protocol_version_switch_with_many_seats() {
    ...
    let validators = vec![
        power("test1".parse().unwrap(), amount_powered),
        power("test2".parse().unwrap(), amount_powered / 5),
    ];
    let mut epoch_manager =
...
fn test_epoch_validators_cache() {
    ...
    let amount_powered = 1_000_000;
    let power_validators: Vec<(AccountId, u64)> = vec![
        ("test1".parse().unwrap(), amount_powered),
        ("test2".parse().unwrap(), amount_powered),
    ];
...
fn test_chunk_producers() {
    let amount_pledged = 1_000_000;
    // Make sure that last validator has at least 160/1'000'000  / num_shards of pledge.
    ...
    let block_producers = epoch_manager
        ...
        .unwrap()
        .iter()
        .map(|(pledge, _)| pledge.account_id().to_string())
        .collect::<Vec<_>>();
    ...
    let mut chunk_producers = epoch_manager
        ...
        .to_vec()
        .iter()
        .map(|pledge| pledge.account_id().to_string())
        .collect::<Vec<_>>();
...
fn test_validator_kickout_sanity() {
    ...
    let accounts = vec![
        ("test0".parse().unwrap(), 1000, 1000),
        ("test1".parse().unwrap(), 1000, 1000),
        ("test2".parse().unwrap(), 1000, 1000),
        ("test3".parse().unwrap(), 1000, 1000),
        ("test4".parse().unwrap(), 500, 500),
    ];
...
#[test]
/// Test that the pledge of validators kicked out in an epoch doesn't exceed the max_kickout_pledge_ratio
fn test_max_kickout_pledge_ratio() {
    ...
    let prev_validator_kickout =
        HashMap::from([("test3".parse().unwrap(), ValidatorKickoutReason::Unpowered)]);
    let (kickouts, validator_stats) = EpochManager::compute_kickout_info(
        &epoch_config,
        &epoch_info,
        &block_stats,
    ...
    // At most 50% of total pledge can be kicked out
    epoch_config.validator_max_kickout_pledge_perc = 40;
    let (kickouts, validator_stats) = EpochManager::compute_kickout_info(
        &epoch_config,
        &epoch_info,
        &block_stats,
