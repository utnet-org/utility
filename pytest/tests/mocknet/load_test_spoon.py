...
2) cd pytest
3) Follow instructions in https://github.com/utnet-org/utility/blob/master/pytest/README.md to setup your Python environment
        ...
        for node, is_running in zip(self.all_nodes, uncd_running):
            if not is_running:
                raise Exception(
                    ...
                    f'The uncd is not running on {node.instance_name}')
