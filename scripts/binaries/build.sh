...
if ! test -f "$DOCKER_FILE"; then
    echo "Unknown option"
    echo "Supported DISTRO: ubuntu-2004 ubuntu-2204 ubuntu-2404 debian-11 debian-12 arch"
...
# Build the image
"$ENGINE" build -t uncnodet --ignorefile=$DOCKER_IGNORE_FILE -f $DOCKER_FILE .


selinuxenabled
if [ $? -ne 0 ]; then
    "$ENGINE" run --rm -v ./output/$ARCH/$DISTRO:/output uncnodet
else
    "$ENGINE" run --rm -v ./output/$ARCH/$DISTRO:/output:z uncnodet
fi
...
if  [[ "$ZIP" ]]; then
    ...
    tar -czf ./../../../output/zips/${ARCH}-${DISTRO}-unc-node.tar.gz unc-node
fi
