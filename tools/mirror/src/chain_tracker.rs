...
use unc_indexer_primitives::{IndexerExecutionOutcomeWithReceipt, IndexerTransactionWithOutcome};
use unc_primitives::hash::CryptoHash;
use unc_primitives::transaction::Transaction;
...
impl TxTracker {
    ...
    async fn on_target_block_applied_receipt(
        ...
        pledged_accounts: &mut HashMap<(AccountId, PublicKey), AccountId>,
    ) -> anyhow::Result<()> {
        ...
        for receipt_id in outcome.execution_outcome.outcome.receipt_ids {
            ...
            // to keep track of which receipts are descendants of our transactions, so that we can reverse any pledge actions
        ...
        match outcome.receipt.receipt {
            ReceiptEnumView::Action { actions, .. } => {
                // since this receipt was recorded in the DB, it means that this pledge action
    ...
    // return value maps (receiver_id, pledging public key) to the predecessor_id in the
    // receipt for any receipts that contain pledge actions (w/ nonzero pledge) that were
    // generated by our transactions. Then the caller will send extra pledge transactions
    // to reverse those.
    pub(crate) async fn on_target_block(
        ...
        let mut pledged_accounts = HashMap::new();
        for s in msg.shards {
            if let Some(c) = s.chunk {
    ...
    async fn on_tx_sent(
        ...
        match &tx_ref {
            ...
            None => {
                // if tx_ref is None, it was an extra tx we sent to unpledge an unwanted validator,
    ...
    pub(crate) async fn on_txs_sent(
        ...
        let (txs_sent, provenance) = match sent_batch {
            ...
            SentBatch::ExtraTxs(txs) => (
                ...
                String::from("extra unpledge transactions"),
            ),
        };
