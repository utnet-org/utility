...
pub fn undo_block(
    ...
    tracing::info!(target: "uncd", ?prev_block_hash, ?current_head_hash, ?prev_block_height, ?current_head_height, "Trying to update head");
    ...
    tracing::info!(target: "uncd", ?new_head_height, ?new_header_height, "The current chain store shows");
    Ok(())
}
...
pub fn undo_only_block_head(
    ...
    tracing::info!(target: "uncd", ?tail_height, ?current_head_height, ?current_header_height, "Trying to update head");

    if current_head_height == tail_height {
        tracing::info!(target: "uncd", "Body head is alreay at the oldest block.");
        return Ok(());
    }
