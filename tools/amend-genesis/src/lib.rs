        ...
        <S as SerializeSeq>::Error: Send + Sync + 'static,
    {
        match self.account {
            Some(mut account) => {
                ...
                if self.amount_needed {
                    account.set_amount(10_000 * unc_infra::config::UNC_BASE);
                }
    ...
    impl TestCase {
        fn parse(&self) -> anyhow::Result<ParsedTestCase> {
            ...
            let genesis_config = GenesisConfig {
                ...
                genesis_height: 0,
                num_block_producer_seats: unc_infra::config::NUM_BLOCK_PRODUCER_SEATS,
                num_block_producer_seats_per_shard: utils::get_num_seats_per_shard(
                    num_shards,
                    unc_infra::config::NUM_BLOCK_PRODUCER_SEATS,
                ),
                avg_hidden_validator_seats_per_shard: (0..num_shards).map(|_| 0).collect(),
                protocol_upgrade_pledge_threshold:
                    unc_infra::config::PROTOCOL_UPGRADE_STAKE_THRESHOLD,
                epoch_length: 1000,
                gas_limit: unc_infra::config::INITIAL_GAS_LIMIT,
                gas_price_adjustment_rate: unc_infra::config::GAS_PRICE_ADJUSTMENT_RATE,
                block_producer_kickout_threshold:
                    unc_infra::config::BLOCK_PRODUCER_KICKOUT_THRESHOLD,
                chunk_producer_kickout_threshold:
                    unc_infra::config::CHUNK_PRODUCER_KICKOUT_THRESHOLD,
                online_max_threshold: Rational32::new(99, 100),
                online_min_threshold: Rational32::new(
                    unc_infra::config::BLOCK_PRODUCER_KICKOUT_THRESHOLD as i32,
                    100,
                ),
                validators: initial_validators,
                transaction_validity_period: unc_infra::config::TRANSACTION_VALIDITY_PERIOD,
                protocol_reward_rate: unc_infra::config::PROTOCOL_REWARD_RATE,
                max_inflation_rate: unc_infra::config::MAX_INFLATION_RATE,
                total_supply: get_initial_supply(&records_in),
                num_blocks_per_year: unc_infra::config::NUM_BLOCKS_PER_YEAR,
                protocol_treasury_account: "treasury.unc".parse().unwrap(),
                fishermen_threshold: unc_infra::config::FISHERMEN_THRESHOLD,
                shard_layout: shards,
                min_gas_price: unc_infra::config::MIN_GAS_PRICE,
                ..Default::default()
            };
    ...
    static TEST_CASES: &[TestCase] = &[
        ...
        TestCase {
            ...
            wanted_records: &[
                ...
                TestStateRecord::Account {
                    account_id: "foo2",
                    amount: 10_000 * unc_infra::config::UNC_BASE,
                    pledging: 3_000_000,
                    power: 3_000_000,
                    storage_usage: 182,
                },
