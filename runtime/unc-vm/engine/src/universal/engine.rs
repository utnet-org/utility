...
impl UniversalEngine {
    ...
    pub fn compile_universal(
        ...
        let environ = unc_vm_compiler::ModuleEnvironment::new();
        let translation = environ.translate(binary).map_err(CompileError::Wasm)?;

        let memory_styles: PrimaryMap<unc_vm_types::MemoryIndex, _> = translation
            .module
            .memories
            .values()
            .map(|memory_type| tunables.memory_style(memory_type))
            .collect();
        let table_styles: PrimaryMap<unc_vm_types::TableIndex, _> = translation
            .module
            .tables
            .values()
        ...
        // Compile the Module
        let compile_info = unc_vm_compiler::CompileModuleInfo {
            module: Arc::new(translation.module),
            features: features.clone(),
            memory_styles,
            table_styles,
        };
        let unc_vm_compiler::Compilation {
            functions,
            custom_sections,
        ...
        let data_initializers = translation
            .data_initializers
            .iter()
            .map(unc_vm_types::OwnedDataInitializer::new)
            .collect();
    ...
    pub fn load_universal_executable(
        ...
        let imports = module
            .imports
            .iter()
            .map(|((module_name, field, idx), entity)| unc_vm_vm::VMImport {
