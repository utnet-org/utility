...
impl<'data> ModuleEnvironment<'data> {
    ...
    pub(crate) fn declare_passive_element(
        ...
        debug_assert!(
            old.is_none(),
            "should never get duplicate element indices, that would be a bug in `unc_vm_compiler`'s \
             translation"
        );
        Ok(())
    }
    ...
    pub(crate) fn declare_passive_data(
        ...
        debug_assert!(
            old.is_none(),
            "a module can't have duplicate indices, this would be a unc_vm-compiler bug"
        );
        Ok(())
    }
