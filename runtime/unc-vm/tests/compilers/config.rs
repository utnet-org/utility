use unc_vm_compiler::{CompilerConfig, Features};
use unc_vm_engine::universal::UniversalEngine;
use unc_vm_test_api::Store;
...
impl Config {
    ...
    pub fn engine(&self, compiler_config: Box<dyn CompilerConfig>) -> UniversalEngine {
        let mut engine = unc_vm_engine::universal::Universal::new(compiler_config)
            .code_memory_pool(
                unc_vm_engine::universal::LimitedMemoryPool::new(128, 16 * 4096).unwrap(),
            );
    ...
    pub fn engine_headless(&self) -> UniversalEngine {
        unc_vm_engine::universal::Universal::headless().engine()
    }

    pub fn compiler_config(&self, canonicalize_nans: bool) -> Box<dyn CompilerConfig> {
        match &self.compiler {
            Compiler::Singlepass => {
                let mut compiler = unc_vm_compiler_singlepass::Singlepass::new();
