...
impl Compiler for SinglepassCompiler {
    ...
    #[tracing::instrument(target = "unc_vm", level = "info", skip_all)]
    fn compile_module(
        &self,
        target: &Target,
        ...
        let import_trampolines: PrimaryMap<SectionIndex, _> =
            tracing::debug_span!(target: "unc_vm", "import_trampolines", n_imports = import_idxs.len()).in_scope(
                || {
                    import_idxs
                        .into_par_iter()
        ...
        let functions = function_body_inputs
            ...
            .map_init(make_assembler, |assembler, (i, input)| {
                tracing::debug_span!(target: "unc_vm", "function", i = i.index()).in_scope(|| {
                    let reader =
                        unc_vm_compiler::FunctionReader::new(input.module_offset, input.data);
                    ...
                    while generator.has_control_frames() {
                        let (op, pos) =
                            tracing::trace_span!(target: "unc_vm", "parsing-next-operator")
        ...
        let function_call_trampolines =
            tracing::debug_span!(target: "unc_vm", "function_call_trampolines").in_scope(|| {
                module
                    .signatures
                    .values()
                    .collect::<Vec<_>>()
