...
use crate::{unc_vm_call_trampoline, Artifact, VMOffsets, VMTrampoline};
...
impl InstanceHandle {
    ...
    /// the `unc_vm` crate API rather than this type since that is vetted for
    /// safety.
    ///
    ...
    /// See [`traphandlers::unc_vm_call_trampoline`].
    pub unsafe fn invoke_function(
        ...
        unc_vm_call_trampoline(vmctx, trampoline, callee, values_vec)
    }
...
///   visible to code in `unc_vm_vm`, so it's the caller's responsibility to ensure these
///   functions are called with the correct type.
/// - `instance_ptr` must point to a valid `unc_vm_test_api::Instance`.
