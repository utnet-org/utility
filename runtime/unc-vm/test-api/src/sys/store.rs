        ...
        fn get_config() -> impl CompilerConfig + 'static {
            cfg_if::cfg_if! {
                if #[cfg(feature = "default-singlepass")] {
                    unc_vm_compiler_singlepass::Singlepass::default()
                } else {
        ...
        fn get_engine(mut config: impl CompilerConfig + 'static) -> UniversalEngine {
            cfg_if::cfg_if! {
                if #[cfg(feature = "default-universal")] {
                    let pool = unc_vm_engine::universal::LimitedMemoryPool::new(1, 0x10000).unwrap();
                    unc_vm_engine::universal::Universal::new(config)
                        .code_memory_pool(pool)
                        .engine()
                } else if #[cfg(feature = "default-dylib")] {
                    unc_vm_engine_dylib::Dylib::new(config)
                        .engine()
                } else {
