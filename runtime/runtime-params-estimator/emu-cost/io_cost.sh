...
cargo build --release --package unc-node
...
dir=/tmp/data$acc
rm -rf $dir
$base/../../target/release/unc-node  --home $dir init \
    --test-seed=alice --account-id=test --fast
...
#cat $log | python3 ./data_builder.py | feedgnuplot --domain --dataid --lines --points  --title "Instructions to IO"  --unset grid  --autolegend
