        ...
        let stored_artifact: Option<VMArtifact> = match cache_record {
            ...
            Some(CompiledContract::Code(serialized_module)) => {
                ...
                unsafe {
                    ...
                    // cache key when upgrading the version of the unc_vm library or the database could
                    // have had its data corrupted while at rest.
                    //
                    // There should definitely be some validation in unc_vm to ensure we load what we think
                    // we load.
