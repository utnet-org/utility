...
done for the purpose of testing changes to unc-node itself, but it's also
...
`unc-node localnet` command, except with account balances and data taken
from mainnet
...
```shell
$ unc-node --home $UNC_HOME_DIRECTORY view-state dump-state --stream
```
...
```shell
$ unc-node --home ~/unc-test-chain/validator0 init --account-id validator0.unc
$ unc-node --home ~/unc-test-chain/validator1 init --account-id validator1.unc
```
...
use the `unc-node amend-genesis` command like so:

```shell
# mkdir ~/unc-test-chain/
$ unc-node amend-genesis --genesis-file-in $UNC_HOME_DIRECTORY/output/genesis.json --records-file-in $UNC_HOME_DIRECTORY/output/records.json --validators ~/test-chain-scratch/validators.json --extra-records ~/test-chain-scratch/extra-records.json --chain-id $TEST_CHAIN_ID --records-file-out ~/unc-test-chain/records.json --genesis-file-out ~/unc-test-chain/genesis.json
```
...
After making these changes, you can try running one unc-node process for
each of your validators:

```shell
$ unc-node --home ~/unc-test-chain/validator0 run
$ unc-node --home ~/unc-test-chain/validator1 run
```
